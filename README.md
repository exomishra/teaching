# Teaching

This repository serves as a lightweight index of my teaching material across
deep learning topics.

Each course lives in its own dedicated repository to keep content
self-contained, versioned, and easy to reuse.

---

## Courses

| Course | Year | Duration | Location | Repository |
|------|------|----------|----------|------------|
| Computer Vision | 2025 | 2 days | CAS ML 6 HSLU | [link](https://github.com/exomishra/computer-vision-cas-hslu-2025) |
| Cross Attention in Vision Language Models| 2026 | 1 day | CAS GenAI Armasuisse | [link](https://github.com/exomishra/attention-VLM-2026/tree/main) |
| Agent Ops (MLOps, MCP, Evaluations) | 2026 | 2 days | [SAS Bootcamp HSLU](https://www.hslu.ch/de-ch/informatik/weiterbildung/applied-data-intelligence/sas-llms-and-ai-agents/) | _TBD_ |

---

## Notes
- Each course repository contains its own slides, notebooks, assignments, and setup instructions.
- This repository is intentionally kept minimal and only acts as a navigation entry point.
